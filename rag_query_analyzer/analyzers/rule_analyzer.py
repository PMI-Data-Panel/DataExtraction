...
class RuleBasedAnalyzer(BaseAnalyzer):
    ...
    def _init_meta_keywords(self) -> set:
        ...
        return {
            ...
            # 일반적인 연결어/동사 (검색에 도움 안 됨)
            '있는', '있다', '있음', '없는', '없다', '없음',
            '하는', '하다', '함', '한', '했', '할', '하고', '하며', '하면서',
            '되는', '되다', '됨', '된', '됐', '될',
            '경험', '경험이', '경험을', '경험은',
            '마신', '마시는', '마시다', '마셔', '마시고',
            '피운', '피우는', '피우다', '피워', '피우고',
    ...
    def _extract_behavioral_conditions(self, query: str) -> Tuple[Dict[str, bool], Set[str]]:
        ...
        specs = {
            "smoker": {
                "negative": [
                    r'비\s*흡연자?',
                    r'흡연(을|를)?\s*(안|않|하지|안함|않음)',
                    r'담배(을|를)?\s*(안|않)\s*피',
                    r'담배(를|을)\s*피워본\s*적\s*(이|가)\s*없',
                    r'금연자?',
                    r'non[-\s]?smoker',
                    r'담배\s*안\s*피',
                ],
                "positive": [
                    ...
                    r'흡연\s*중',
                    r'흡연(을|를)?\s*하는',
                    r'흡연(을|를)?\s*하고',      # "smoking and" - conjunction form
                    r'흡연(을|를)?\s*하며',      # "while smoking"
                    r'흡연(을|를)?\s*하면서',    # "while smoking"
                    r'흡연(을|를)?\s*하거나',    # "smoking or"
                    r'담배(를|을)?\s*(피우|피는|피운|피움|피고|피며|피면서|피거나)',
                    r'smoker',
                ],
            ...
            "has_vehicle": {
                "negative": [
                    r'차량(이|가)?\s*(없|미보유|미소유)',
                    r'자동차(이|가)?\s*(없|미보유|미소유)',
                    r'차(가|이)?\s*없',
                    r'무\s*차량',
                    r'차량\s*보유\s*(안|않)',
                    r'보유\s*차량(이|가)?\s*없',  # "보유 차량이 없"
                    r'차량(을|를)?\s*보유\s*(안|하지\s*않|못)',
                ],
                "positive": [
                    r'차량(이|가)?\s*(있|보유|소유)',
                    r'자동차(이|가)?\s*(있|보유|소유)',
            ...
            "drinker": {
                "positive": [
                    r'술(을|를)?\s*(마시|마신|마셔|음용)',
                    r'술\s*.*\s*(경험|있)',
                    r'음주(를|을)?\s*(경험|있|하는|하고|하며)',
                    r'음주자',
                    r'drinker',
                ],
                "token_keywords": ("술", "음주", "마신", "마시는"),
            },
            "non_drinker": {
                "positive": [
                    r'술(을|를)?\s*(안|않)\s*마',
